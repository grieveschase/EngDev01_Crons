...
def write_error_log():
    with open(os.path.split(__file__)[0] + "//manual_image_move_error_log.txt","a") as o_file:
...
def write_app_log(app_text):
    with open(os.path.split(__file__)[0] + "//manual_image_move_log.txt","a") as o_file:
...
for tool, path in img_vars_config.tool_path_manual_img_dict.items():
    try:
        ...
        write_error_log()
        subject = "Manual SEM Image Mover Error"
        body ="Error with Man Img sweeper"
